...
def test_files():
    ...
    assert Path("handcoded_html/users/awdeorio/index.html").exists()
    assert Path(
        "handcoded_html/uploads/505083b8b56c97429a728b68f31b0b2a089e5113.jpg"
    ).exists()
    assert Path(
        "handcoded_html/uploads/5ecde7677b83304132cb2871516ea50032ff7a4f.jpg"
    ).exists()
    assert Path(
        "handcoded_html/uploads/73ab33bd357c3fd42292487b825880958c595655.jpg"
    ).exists()
    assert Path(
        "handcoded_html/uploads/ad7790405c539894d25ab8dcf0b79eed3341e109.jpg"
    ).exists()
    assert Path(
        "handcoded_html/uploads/9887e06812ef434d291e4936417d125cd594b38a.jpg"
    ).exists()
    assert Path(
        "handcoded_html/uploads/e1a7c5c32973862ee15173b0259e3efdb6a391af.jpg"
    ).exists()
    assert Path(
        "handcoded_html/uploads/2ec7cf8ae158b3b1f40065abfb33e81143707842.jpg"
    ).exists()
    assert Path(
        "handcoded_html/uploads/122a7d27ca1d7420a1072f695d9290fad4501a41.jpg"
    ).exists()


def test_images():
    """Verify all images are present in /index.html."""
    with open("handcoded_html/index.html", encoding='utf-8') as infile:
